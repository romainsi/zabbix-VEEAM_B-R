...
switch ($ITEM)
{
	...
	"DiscoveryAgentJobs" {
		$xml1 = ImportXml -item backupjob | Where-Object { $_.IsScheduleEnabled -eq "true" -and $_.JobType -like "EpAgentBackup" }
		$query = $xml1 | Where-Object { $_.IsScheduleEnabled -eq "true" -and $_.JobType -like "EpAgentBackup" } | Select Id, Name | Select @{ N = "JOBAGENTID"; E = { $_.ID | convertto-encoding -switch in } }, @{ N = "JOBAGENTNAME"; E = { $_.NAME | convertto-encoding -switch in } }
	...
	"ResultAgentBackup"  {
		$query = [veeam.backup.core.cbackupsession]::GetByJob("$ID") | Where-Object { ($_.JobType -match 'EpAgent') } | Sort creationtime -Descending | Select -First 1
		$result = [String]$query.Result | veeam-replace
		write-host "$result"
	}
